...
func (b *Backend) ensureShare(ctx context.Context, mountpoint, name string) error {
	...
	if share != nil && !share.serving() {
		...
		// appliance: sharing.smb.update accepts {"enabled": false} and the
		...
		return status.Errorf(codes.FailedPrecondition,
			"the SMB share for %s is disabled on the appliance, so a volume using it "+
...
func (b *Backend) AcceptedParameters() []string {
	return []string{"server", "shareName", "secretName", "secretNamespace", "dirMode", "fileMode", "mode", "uid", "gid"}
}
