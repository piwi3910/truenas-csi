...
func (b *Backend) Create(ctx context.Context, r backend.CreateRequest) (*backend.Volume, error) {
	...
	// guess -- see backend.AbandonedCloneOf. Without this the retry compared
	// refquota 0 against the requested size and answered AlreadyExists forever,
	// because the provisioner retries a deterministic failure.
	resuming := backend.AbandonedCloneOf(existing, r)
