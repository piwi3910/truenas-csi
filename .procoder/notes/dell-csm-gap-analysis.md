...
### 4.4 Four protocols in one driver — with one honest asterisk

PowerScale is NFS-only, PowerFlex is SDC plus NVMe/TCP, Unity has no NVMe. We
serve NFS, iSCSI and NVMe/TCP from one binary, and **SMB has no analogue
anywhere in CSM** — no Dell driver speaks it.

**Correction, found 2026-09-08 while building the E2E harness.** An earlier
draft claimed all four protocols worked end to end. SMB did not. It provisioned
correctly, but no pod could consume it: `internal/node` had zero references to
`smb` or `cifs`, the preflight had no cifs capability so no node published an
`smb` topology label, and `requiredTopology()` demanded exactly that label — so
an SMB PVC was unschedulable before it was unmountable. `smb_test.go` missed it
by issuing its own `mount -t cifs` and never calling `NodeStageVolume`: the same
shape of mistake as the mount-propagation bug, where a test verified the pod's
own writes and concluded the volume was remote.

Fixed since: `internal/node/cifs.go` is the data path, `CapSMB` the preflight
entry, and the access-mode test is now shared-filesystem vs raw-block rather
than a list of protocol names, so SMB gets the RWX it always supported.
