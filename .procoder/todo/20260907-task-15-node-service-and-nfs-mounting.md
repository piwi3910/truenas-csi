...
- [ ] Write `TestE2ENFSMountLifecycle` with a recording `Executor`: assert `NodeStageVolume` issues exactly `mount -t nfs -o vers=4 <server>:<share> <staging path>`, that `nfsVersion: "3"` in the publish context changes `vers=4` to `vers=3`, and that `NodeUnstageVolume` issues `umount <staging path>` and nothing else. Run `go test ./internal/node/` — expect FAIL with "undefined: NewNode".
- [ ] Write `TestNFSStageIsIdempotent` asserting a second `NodeStageVolume` for an already-mounted path issues no second mount and returns success.
- [ ] Write `TestNFSUnstageAbsentMountSucceeds` asserting unstaging a path that is not mounted returns success and issues no umount.
- [ ] Write `TestNodePublishBindMounts` asserting `NodePublishVolume` bind-mounts the staging path to the target path and honours `readonly: true` with the `ro` option.
- [ ] Implement `Executor` with a host-namespace implementation using `nsenter --mount=<root>/proc/1/ns/mnt`.
- [ ] Implement NFS stage as an idempotent mount: check the mount table first, mount only when absent, create the staging directory when missing.
- [ ] Implement `NodeGetInfo` returning the configured node id, the topology labels from Task 14, and `max_volumes_per_node` of 128.
